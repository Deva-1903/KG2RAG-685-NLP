...
def calculate_ci(mean: float, std: float, n: int, confidence: float = 0.95) -> Tuple[float, float]:
    """
    ...
    # For large samples (n > 30), use z-distribution (1.96 for 95% CI)
    # For small samples, approximate t-distribution
    if n > 30:
        z_score = 1.96  # 95% CI
    else:
        # Approximate t-values for small samples
        # For 95% CI: df=10->2.228, df=20->2.086, df=30->2.042
        df = n - 1
        if df <= 10:
            t_score = 2.228
        elif df <= 20:
...
def comprehensive_evaluate(prediction_file: str, gold_file: str, 
    ...
    if n > 0:
        # Answer metrics CIs
        answer_em_values = [q['em'] for q in answer_per_q]
        answer_f1_values = [q['f1'] for q in answer_per_q]
        answer_std_em = statistics.stdev(answer_em_values) if len(answer_em_values) > 1 else 0
        answer_std_f1 = statistics.stdev(answer_f1_values) if len(answer_f1_values) > 1 else 0
        answer_ci_em = calculate_ci(answer_metrics['em'], answer_std_em, n)
        answer_ci_f1 = calculate_ci(answer_metrics['f1'], answer_std_f1, n)
        
        # Supporting facts metrics CIs
        sp_em_values = [q['sp_em'] for q in sp_per_q]
        sp_f1_values = [q['sp_f1'] for q in sp_per_q]
        sp_std_em = statistics.stdev(sp_em_values) if len(sp_em_values) > 1 else 0
        sp_std_f1 = statistics.stdev(sp_f1_values) if len(sp_f1_values) > 1 else 0
        sp_ci_em = calculate_ci(sp_metrics['sp_em'], sp_std_em, n)
        sp_ci_f1 = calculate_ci(sp_metrics['sp_f1'], sp_std_f1, n)
        
        # Joint metrics CIs
        joint_em_values = [q['joint_em'] for q in joint_per_q]
        joint_f1_values = [q['joint_f1'] for q in joint_per_q]
        joint_std_em = statistics.stdev(joint_em_values) if len(joint_em_values) > 1 else 0
        joint_std_f1 = statistics.stdev(joint_f1_values) if len(joint_f1_values) > 1 else 0
        joint_ci_em = calculate_ci(joint_metrics['joint_em'], joint_std_em, n)
        joint_ci_f1 = calculate_ci(joint_metrics['joint_f1'], joint_std_f1, n)
    else:
        answer_ci_em = answer_ci_f1 = (0, 0)
        sp_ci_em = sp_ci_f1 = (0, 0)
        joint_ci_em = joint_ci_f1 = (0, 0)
    
    # Compile results
    results = {
        'answer_metrics': {
            **answer_metrics,
            'ci_95_em': {'lower': answer_ci_em[0], 'upper': answer_ci_em[1]},
            'ci_95_f1': {'lower': answer_ci_f1[0], 'upper': answer_ci_f1[1]}
        },
        'supporting_facts_metrics': {
            **sp_metrics,
            'ci_95_em': {'lower': sp_ci_em[0], 'upper': sp_ci_em[1]},
            'ci_95_f1': {'lower': sp_ci_f1[0], 'upper': sp_ci_f1[1]}
        },
        'joint_metrics': {
            **joint_metrics,
            'ci_95_em': {'lower': joint_ci_em[0], 'upper': joint_ci_em[1]},
            'ci_95_f1': {'lower': joint_ci_f1[0], 'upper': joint_ci_f1[1]}
        },
    ...
    print("="*80)
    print(f"\nAnswer Quality:")
    print(f"  EM: {answer_metrics['em']:.4f} [95% CI: {answer_ci_em[0]:.4f}, {answer_ci_em[1]:.4f}]")
    print(f"  F1: {answer_metrics['f1']:.4f} [95% CI: {answer_ci_f1[0]:.4f}, {answer_ci_f1[1]:.4f}]")
    print(f"  Precision: {answer_metrics['prec']:.4f}")
    print(f"  Recall: {answer_metrics['recall']:.4f}")
    
    print(f"\nSupporting Facts Quality:")
    print(f"  EM: {sp_metrics['sp_em']:.4f} [95% CI: {sp_ci_em[0]:.4f}, {sp_ci_em[1]:.4f}]")
    print(f"  F1: {sp_metrics['sp_f1']:.4f} [95% CI: {sp_ci_f1[0]:.4f}, {sp_ci_f1[1]:.4f}]")
    print(f"  Precision: {sp_metrics['sp_prec']:.4f}")
    print(f"  Recall: {sp_metrics['sp_recall']:.4f}")
    
    print(f"\nJoint Metrics (Answer + Supporting Facts):")
    print(f"  EM: {joint_metrics['joint_em']:.4f} [95% CI: {joint_ci_em[0]:.4f}, {joint_ci_em[1]:.4f}]")
    print(f"  F1: {joint_metrics['joint_f1']:.4f} [95% CI: {joint_ci_f1[0]:.4f}, {joint_ci_f1[1]:.4f}]")
