...
def run_batch_experiment(num_questions, batch_id, total_batches, base_output_dir="../output/hotpot", seed=None):
    ...
    # Script paths (in experimental directory)
    script_dir = os.path.dirname(os.path.abspath(__file__))
    original_script = os.path.join(script_dir, 'kg_rag_baseline.py')
    experimental_script = os.path.join(script_dir, 'kg_rag_enhanced.py')
    
    # Run original pipeline (baseline)
    print(f"\n[1/2] Running Baseline KG²RAG Pipeline (Batch {batch_id})...")
    original_success = run_pipeline(original_script, original_args, original_file)
    ...
    experimental_success = run_pipeline(experimental_script, experimental_args, experimental_file)
