...
Supports testing on N questions (configurable via --num_questions).
"""
...
def process_sample_enhanced(args, sample, kg, subquestion_generator):
    ...
    # Retrieve passages for each sub-question separately, then fuse rankings
    # This ensures coverage of all reasoning hops before KG expansion
    multi_view_retriever = MultiViewRetriever(
        index=index,                        # Vector index for semantic retrieval
        fusion_method=args.fusion_method,   # "rrf" (Reciprocal Rank Fusion) or "mean" (mean-cosine)
        top_n_per_view=args.top_n_per_view, # Number of passages retrieved per sub-question
        final_top_k=args.seed_top_k,        # Final number of seed passages after fusion/reranking
        device='cpu'                        # Device for cross-encoder (CPU or CUDA)
    )
    
    # Retrieve diverse seed passages using multi-view approach
    # Returns: List of (doc_id, score, text) tuples
    seed_passages = multi_view_retriever.retrieve_seeds(
        main_question=sample_question,  # Original question (used for reranking)
        sub_questions=sub_questions     # Generated sub-questions (one retrieval per sub-q)
    )
    print(f'Sample {sample_id}: Retrieved {len(seed_passages)} seed passages via multi-view retrieval')
    ...
    # Cap pool size to ≤ 60 chunks as per proposal (computational efficiency)
    # This ensures knapsack solver remains tractable while maintaining diversity
    MAX_POOL_SIZE = 60
    if len(candidates) > MAX_POOL_SIZE:
        # Prioritize seed passages (from multi-view retrieval) over expanded passages
        # Seeds are higher quality, so we keep all seeds and fill remaining slots with expanded
        ...
        if seed_candidates:
            ...
            # No seeds found (unlikely), just take first MAX_POOL_SIZE
    ...
    # Extract entities from question for coverage calculation
    question_entities = extract_entities(sample_question)
    
    # Extract keywords from sub-questions for additional context
    # (Note: coverage bonus currently uses only question entities, but keywords kept for compatibility)
    subquestion_keywords = set()
    for sq in sub_questions:
        subquestion_keywords.update(sq.lower().split())

    # Initialize knapsack selector with token budget
    # use_dp=True uses exact dynamic programming, False uses greedy approximation
    knapsack_selector = KnapsackSelector(
        token_budget=args.token_budget,  # Maximum tokens for selected passages
        use_dp=args.use_dp_knapsack,    # Use exact DP solver or greedy
        device='cpu'                     # Device for cross-encoder (CPU or CUDA)
    )

    # Solve knapsack problem: maximize value (relevance × coverage) under token budget
    # Value = cross_encoder_score × (1 + coverage_bonus)
    # Cost = token_count
    selected_doc_ids, selected_values, total_value = knapsack_selector.select_evidence(
        question=sample_question,
        candidates=candidates,  # List of (doc_id, text) tuples from seeds + KG expansion
        question_entities=question_entities,  # For coverage calculation
        subquestion_keywords=subquestion_keywords  # For compatibility (not currently used)
    )
    
    print(f'Sample {sample_id}: Selected {len(selected_doc_ids)} passages via knapsack (total value: {total_value:.2f})')
